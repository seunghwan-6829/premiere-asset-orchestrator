...
export function createExportPackageManifest(
  projectName: string,
  segments: SubtitleSegment[],
  selectedIds: string[],
  recommendations: RecommendationDraft[],
  ...
  const selectedItems = recommendations.filter((item) => selectedIds.includes(item.id));

  return {
    projectName,
    exportedAt: new Date().toISOString(),
    aspectRatio: settings.aspectRatio,
    variantsPerSegment: Math.max(1, settings.variantsPerSegment || 1),
    selectedCount: selectedItems.length,
    items: selectedItems.map((item) => {
      const segment = segments.find((current) => current.id === item.segmentId);

      return {
        segmentId: item.segmentId,
        startTimecode: segment?.startTimecode ?? item.timecode,
        endTimecode: segment?.endTimecode ?? item.timecode,
        sourceText: segment?.text ?? item.title,
        kind: item.kind,
        title: item.title,
  ...
  } satisfies ExportPackageManifest;
}
