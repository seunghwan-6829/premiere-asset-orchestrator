...
type RecommendationApiResponse = {
  error?: string;
  items?: ClaudeRecommendationPayload[];
  mode?: "ai" | "fallback";
};
...
const T = {
  ...
  aiSetupRequired: "서버 환경변수 없이 Claude를 쓰려면 이 세션에만 API 키를 입력해 주세요.",
  localMode: "브라우저 저장 모드",
  localModeHelp: "현재 프로젝트와 임시 API 키는 이 브라우저에 저장됩니다. 팀 공유나 영구 보관은 아직 포함되지 않습니다."
} as const;
...
async function parseJsonSafely(response: Response) {
  ...
  try {
    return JSON.parse(raw) as RecommendationApiResponse;
  } catch {
    throw new Error(raw.slice(0, 280));
  }
}

function toDownloadName(value: string, extension: string) {
  const base = value.trim() || "premiere-project";
...
export default function DashboardApp() {
  ...
  const serverAnthropicReady = hasServerAnthropicKey();
  ...
  const providerStorageKey = session?.user.email ? `premiere-session-keys:${session.user.email.toLowerCase()}` : null;
  ...
  useEffect(() => {
    ...
    const raw = window.sessionStorage.getItem(providerStorageKey);
    if (!raw) {
      setApiKeys({});
      return;
    }
  ...
  useEffect(() => {
    ...
    window.sessionStorage.setItem(providerStorageKey, JSON.stringify(apiKeys));
  ...
  async function handleGenerateRecommendations() {
    ...
    try {
      ...
      setBannerMessage(
        serverAnthropicReady || apiKeys.anthropic?.trim()
          ? `추천 엔진이 ${targetSegments.length}개 구간을 분석 중입니다.`
          : `기본 추천 엔진이 ${targetSegments.length}개 구간을 정리 중입니다.`
      );
      ...
      if (!response.ok || !payload.items) {
        throw new Error(payload.error || "추천 생성에 실패했습니다.");
      }

      setRecommendations(hydrateRecommendations(payload.items));
      setReviewStep(5);
      setBannerMessage(
        payload.mode === "fallback"
          ? `기본 추천 엔진으로 ${payload.items.length}개 항목을 만들었습니다.`
          : `AI가 ${payload.items.length}개 추천 항목을 만들었습니다.`
      );
    } catch (error) {
  ...
  function handleGenerateAssets() {
    ...
    window.setTimeout(() => {
      ...
      setBannerMessage(`선택한 ${targets.length}개 구간을 export 패키지용 상태로 정리했습니다.`);
    }, 900);
  }
  ...
  function handleExportXml() {
    ...
    downloadTextFile(toDownloadName(selectedProject.name, "xml"), xml, "application/xml;charset=utf-8");
  ...
  function handleExportManifest() {
    ...
    const selectedIds = recommendations.filter((item) => item.decision === "selected").map((item) => item.id);
    if (selectedIds.length === 0) {
      setBannerMessage("JSON으로 내보낼 항목이 없습니다.");
      return;
    }

    const manifest = createExportPackageManifest(selectedProject.name, segments, selectedIds, recommendations, {
      variantsPerSegment: Number(variantsPerSegment) || 1,
      aspectRatio: aspectRatio || "16:9"
    });

    downloadTextFile(
      toDownloadName(`${selectedProject.name}-manifest`, "json"),
      JSON.stringify(manifest, null, 2),
      "application/json;charset=utf-8"
    );
    setBannerMessage("편집 패키지용 JSON 매니페스트를 다운로드했습니다.");
  }
        ...
        {!selectedProject ? (
          <div className="dashboard-stack">
            ...
            <section className="dashboard-grid compact-grid">
              ...
              <article className="dashboard-card panel-surface">
                <p className="section-kicker">{T.localMode}</p>
                <h3>저장 방식</h3>
                <strong>Browser MVP</strong>
                <p>{T.localModeHelp}</p>
              </article>
            </section>
          </div>
            ...
            <aside className="review-column panel-surface">
              <div className="section-header review-header">
                <div>
                  ...
                  <div className="inline-actions review-header-actions">
                    ...
                    <button className="button button-secondary" disabled={reviewStep < 5} onClick={handleExportManifest}>
                      {T.downloadManifest}
                    </button>
              ...
              {reviewStep < 5 && reviewItems.length === 0 ? (
                ...
                <div className="review-empty">
                  ...
                  <p>추천 생성 버튼을 누르면 서버 AI 또는 기본 추천 엔진으로 이 영역이 채워집니다.</p>
                </div>
              )}
            </aside>
          </div>
          ...
          <div className="modal-panel panel-surface" onClick={(event) => event.stopPropagation()}>
            <div className="modal-header">
              <div>
                ...
                <p>{serverAnthropicReady ? "서버 환경변수로 Claude가 연결되어 있습니다." : T.aiSetupRequired}</p>
              </div>
