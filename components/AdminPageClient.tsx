...
export default function AdminPageClient() {
  ...
  if (loading) {
    return <div className="app-loading">관리자 화면을 준비하고 있습니다...</div>;
  }
  ...
  if (!isAdminEmail(session.user.email)) {
    return (
      <main className="admin-page-shell">
        <div className="admin-guard panel-surface">
          <h1>관리자 권한이 없습니다.</h1>
          <p>현재 계정: <strong>{session.user.email}</strong></p>
          <p>허용된 관리자 이메일: <strong>{ADMIN_EMAIL || "NEXT_PUBLIC_ADMIN_EMAIL 미설정"}</strong></p>
    ...
    <main className="admin-page-shell">
      <section className="admin-header panel-surface">
        <div>
          ...
          <h1>운영 상태 확인</h1>
          <p>현재 배포 설정과 브라우저 기반 프로젝트 상태를 빠르게 확인하는 페이지입니다.</p>
        </div>
      ...
      <section className="admin-grid compact-grid">
        ...
        <article className="dashboard-card panel-surface">
          ...
          <p>{missingEnvKeys.length === 0 ? `${projectRef || "프로젝트"} 연결 준비 완료` : missingEnvKeys.join(", ")}</p>
        </article>
        <article className="dashboard-card panel-surface">
